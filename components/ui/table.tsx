...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div
      ...
      className="relative w-full overflow-x-auto rounded-2xl border border-border/70 bg-card shadow-sm shadow-primary/5"
    >
      <table
        data-slot="table"
        className={cn("w-full min-w-[48rem] caption-bottom text-sm", className)}
        {...props}
      />
    </div>
  )
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn(
        "max-w-[18rem] px-4 py-3 align-middle whitespace-nowrap [&:has([role=checkbox])]:pr-0",
        className
      )}
      {...props}
    />
